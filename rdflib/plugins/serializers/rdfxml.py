...
class XMLSerializer(Serializer):
	...
	def serialize(self, stream, base=None, encoding=None, **args):
		...
		#~ The lambda function place an encoder before the strea.write function
		#~  encoding all the string datas in byte datas
		#~ Not good if the originale stream is a StringIO (or similar), since it needs string datas, instead will receive bytes data
		self.write = stream.write #= lambda uni: stream.write(uni.encode(encoding, 'replace'))
...
class PrettyXMLSerializer(Serializer):
	...
	def __init__(self, store, max_depth=3):
		...
		#~ print('\n-----------INIT PrettyXMLSerializer----------')
		#~ print(store.namespace_manager)
		#~ for prefix, namespace in store.namespace_manager.namespaces():
	...
	def serialize(self, stream, base=None, encoding=None, **args):
		...
		self.nm = store.namespace_manager
		self.writer = writer = XMLWriter(stream, self.nm, encoding)
		
		namespaces = {}
		possible = uniq(store.predicates()) + uniq(store.objects(None, RDF.type))
		
		#~ print('store.predicates(): ', list(store.predicates()))
		#~ print('store.objects(None, RDF.type): ', list(store.objects(None, RDF.type)))
		#~ print('possible: ', possible)
		
		for predicate in possible:
			prefix, namespace, local = self.nm.compute_qname(predicate)
			#Retrieve the namespaces used
			namespaces[prefix] = namespace
		
		#~ I add/rewrite the 'rdf' namespace
		namespaces["rdf"] = "http://www.w3.org/1999/02/22-rdf-syntax-ns#"
		
		#~ print('\n-----------PrettyXMLSerializer: Scrivo i namespaces----------')
		#~ import pdb
		#~ pdb.set_trace ()
		#~ 
		#~ print(namespaces)
		
		#~ I send the namespaces to the writer
		writer.push(RDF.RDF)
